...
A collection of useful general-purpose skillz for the Gemini CLI to enhance your development workflow.

## Installation

### Install the entire suite
To install all skillz in one swoop:
```bash
git clone https://github.com/j-steve/gemini-cli-skillz.git ~/.gemini/skillz && gemini skills link ~/.gemini/skillz
```

### Install specific skillz
To install individual skillz (e.g., git-operations):
```bash
...
Here is the current catalog of skillz available in this repository:
...
| **`prompt-engineering-conventions`** | Provides guidelines for writing and adjusting LLM prompts using positive framing, proper constraints, and reasoning blocks. | **Write perfect, robust prompts every time.** Keeps your prompts structured, avoids overcorrection and shouting, and ensures they follow industry-standard best practices. |
| **`python-style`** | Establishes comprehensive Python-specific code style conventions, PEP 8 standards, strict typing, and standard logging patterns. | **Write idiomatic, strictly typed Python.** Enforces standard `logger = logging.getLogger(__name__)` instantiation, modern typing union syntax, private constant naming, and modular function architecture. |
